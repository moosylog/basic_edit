// --- 1. CORE COMPILER DOM ACCESS HOOKS ---
const els = {
...
// --- 2. GLOBAL MATRIX VARIABLE FRAMES ---
let config = null;
let activeLayer = 0;
let inspectorPath = []; 
...
// --- 3. HARDWARE MAPPING SCHEMA TABLES ---
const T = {
    friendly: {
        layers: "Matrix Layers", behaviors: "Global Behaviors", holdTaps: "Hold-Taps", tapDances: "Multi-Taps", modMorphs: "Modifier Rules", combos: "Chords / Combos", macros: "Macros",
        ...
        holdAction: "Hold Action", tapAction: "Tap Action", baseAction: "Base Action", shiftAction: "Shifted Action", step: "Step", settings: "Settings", holdTriggerKeyPositions: "Bilateral Combo Keys", holdTriggerOnRelease: "Trigger On Release", retroTap: "Retro Tap", keepMods: "Keep Modifiers", slowRelease: "Slow Release"
    },
    zmk: {
        layers: ".layers", behaviors: "ZMK Behaviors", holdTaps: "&hold_tap", tapDances: "&tap_dance", modMorphs: "&mod_morph", combos: "combos", macros: "&amp;macro",
        ...
        holdAction: "bindings[0]", tapAction: "bindings[1]", baseAction: "bindings[0]", shiftAction: "bindings[1]", step: "bindings[", settings: "Root Config", holdTriggerKeyPositions: "hold-trigger-key-positions", holdTriggerOnRelease: "hold-trigger-on-release", retroTap: "retro-tap", keepMods: "keep-mods", slowRelease: "slow-release"
    }
};
...
const KEYCODE_LABEL_SYMBOLS = { SEMI:';', SQT:"'", GRAVE:'`', COMMA:',', DOT:'.', FSLH:'/', BSLH:'\\', MINUS:'-', EQUAL:'=', LBKT:'[', RBKT:']' };
...
const PROPERTY_SCHEMA = {
    ...
    holdTriggerOnRelease: { type: 'boolean' }, retroTap: { type: 'boolean' }, slowRelease: { type: 'boolean' }, holdTriggerKeyPositions: { type: 'keyPositionList' },
};

const BEHAVIOR_DEF = {
    "&kp": { name: "Standard Keypress", type: "keycode" }, "&mo": { name: "Momentary Layer", type: "layer" }, "&sl": { name: "Sticky Layer", type: "layer" }, "&sk": { name: "Sticky Modifier", type: "modifier" },
    "&mt": { name: "Mod-Tap", type: "modifier+keycode" }, "&lt": { name: "Layer-Tap", type: "layer+keycode" }, "&to": { name: "To Layer", type: "layer" }, "&tog": { name: "Toggle Layer", type: "layer" },
    "&trans": { name: "Transparent", type: "none" }, "&none": { name: "None / Dead Key", type: "none" }, "none": { name: "None / Dead Key", type: "none" }, "Custom": { name: "Global Behavior / Custom", type: "custom" }
};
...
const GLOVE80_GEO = [
    ...
    {x:0,y:315},{x:70,y:315},{x:140,y:280},{x:210,y:280},{x:280,y:280},{x:350,y:280}, {x:448,y:280,r:20,o:"-168px 0px"},{x:546,y:231,r:30,o:"-266px 0px"},{x:707,y:105,r:45,o:"-427px 0px"},{x:490,y:105,r:-45,o:"490px 0px"},{x:651,y:231,r:-30,o:"329px 0px"},{x:749,y:280,r:-20,o:"231px 0px"},
    {x:840,y:280},{x:910,y:280},{x:980,y:280},{x:1050,y:280},{x:1120,y:315},{x:1190,y:315}, {x:0,y:385},{x:70,y:385},{x:140,y:350},{x:210,y:350},{x:280,y:350}, {x:371,y:378,r:15,o:"-91px 0px"},{x:462,y:350,r:25,o:"-182px 0px"},{x:630,y:224,r:45,o:"-350px 0px"},{x:560,y:224,r:-45,o:"420px 0px"},{x:728,y:350,r:-25,o:"252px 0px"},{x:819,y:378,r:-15,o:"161px 0px"}, {x:910,y:350},{x:980,y:350},{x:1050,y:350},{x:1120,y:385},{x:1190,y:385}
];
...
const GO60_GEO = [
    ...
    {x:140,y:280},{x:210,y:280},{x:280,y:280},{x:875,y:280},{x:945,y:280},{x:1015,y:280}, {x:364,y:287,r:12.5,o:"0px 0px"},{x:448,y:308,r:25,o:"0px 0px"},{x:525,y:346.5,r:37,o:"0px 0px"}, {x:640.5,y:385,r:-37,o:"0px 0px"},{x:710.5,y:336,r:-25,o:"0px 0px"},{x:791,y:301,r:-12.5,o:"0px 0px"}
];

// --- 4. DATA CALCULATION HELPER FUNCTIONS ---
function getKeyboardGeo() { if (!config) return []; return (config.layout?.keys) ? config.layout.keys : (config.keyboard?.toLowerCase() === 'go60' ? GO60_GEO : GLOVE80_GEO); }
function calculateMatrixBounds() { const geo = getKeyboardGeo(); if(!geo || geo.length === 0) return { w: 1260, h: 450 }; let maxX = 0, maxY = 0; geo.forEach(g => { if (g.x > maxX) maxX = g.x; if (Math.abs(g.y) > maxY) maxY = Math.abs(g.y); }); return { w: maxX + 100, h: maxY + 100 }; }
...
function triggerAutoZoom() { if(!config || !autoZoom) return; const rect = els.matrixWrap.getBoundingClientRect(); applyScale(Math.min((rect.width - 60) / matrixBounds.w, (rect.height - 60) / matrixBounds.h)); }
function closePicker() { activePickerTarget = null; els.pickerDrawer.classList.add('translate-y-full'); }
function openPicker(callback) { activePickerTarget = callback; els.pickerDrawer.classList.remove('translate-y-full'); }
function toggleTheme() { document.documentElement.classList.toggle('dark'); renderAll(); }
function getBaseType(val) { return (val === 'Custom' || !BEHAVIOR_DEF[val]) ? 'custom' : 'known'; }

// --- 5. WORKSPACE RENDER LOOP ENGINES ---
function renderSidebar() {
    els.layerList.innerHTML = "";
    config.layers.forEach((_, i) => {
        let name = config.layer_names?.[i] || `Layer ${i}`, isActive = activeLayer === i;
        let row = document.createElement('div'); row.className = `flex items-center gap-1 group rounded-lg p-0.5 transition-all ${isActive ? 'bg-[color:var(--accent-soft)] border border-[color:var(--accent)]':'border-transparent hover:bg-slate-100 dark:hover:bg-slate-800'}`;
        row.innerHTML = `<button class="flex-1 text-left px-2 py-1.5 text-xs font-bold truncate ${isActive?'text-[color:var(--accent)]':'text-slate-500 dark:text-slate-400'}">${name}</button>`;
        row.onclick = () => { activeLayer = i; inspectorPath = []; renderAll(); };
    ...
    [ 'holdTaps', 'tapDances', 'modMorphs', 'macros', 'combos' ].forEach(g => {
        ...
        btn.className = "w-full flex justify-between items-center px-3 py-2 rounded-lg text-xs font-bold transition-all text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800";
...
function renderGenericBindingEditor(bindingObj, context) {
    ...
    (PROPERTY_SCHEMA[behaviorVal] || []).forEach(slot => renderParamSlot(bindingObj, slot, container));
}
...
function renderInspector() {
    ...
    els.inspContent.innerHTML = "";
    if (context.type === 'settings' || context.type === 'matrix') renderGenericBindingEditor(targetData, context);
    else renderSettingsPanel();
}

function renderSettingsPanel() { els.inspContent.innerHTML = `<div class="p-4 bg-white dark:bg-slate-800 border rounded-xl"><h3 class="font-bold text-sm mb-2">Global Settings</h3></div>`; }
...
function updateZoomUI() { els.btnZoomAuto.classList.toggle('text-[color:var(--accent)]', autoZoom); }

// --- 8. IO MOUNT EVENT HANDLERS ---
